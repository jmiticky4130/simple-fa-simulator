...
var $author$project$Main$init = function (_v0) {
	var simulatorInit = $author$project$Pages$Simulator$init(
		{nextStateId: 0, states: _List_Nil, transitions: _List_Nil});
	var editorInit = $author$project$Pages$Editor$init;
	return _Utils_Tuple2(
		{currentPage: $author$project$Main$EditorPage, editorModel: editorInit, simulatorModel: simulatorInit},
...
var $author$project$Utils$AutomatonHelpers$isDFA = F2(
	function (states, transitions) {
		var hasNonDeterminism = A2(
			$elm$core$List$any,
			function (t1) {
				return A2(
					$elm$core$List$any,
					function (t2) {
						return _Utils_eq(t1.from, t2.from) && (_Utils_eq(t1.symbol, t2.symbol) && (!_Utils_eq(t1.to, t2.to)));
					},
					transitions);
			},
			transitions);
		return !hasNonDeterminism;
	});
	...
	function (msg, model) {
		...
		switch (msg.$) {
			case 'SwitchToSimulator':
				return A2($author$project$Utils$AutomatonHelpers$isDFA, currentAutomaton.states, currentAutomaton.transitions) ? _Utils_Tuple2(model, $elm$core$Platform$Cmd$none) : _Utils_Tuple2(
					_Utils_update(
						model,
						{
							consoleMessages: A2(
								$elm$core$List$cons,
								{msgType: $author$project$Components$Console$Error, text: 'Automat nie je DFA (obsahuje nedeterministické prechody).'},
								model.consoleMessages)
						}),
					$elm$core$Platform$Cmd$none);
			case 'Undo':
			...
			case 'ConfirmTransitionSymbol':
				var _v4 = model.editingTransition;
				if (_v4.$ === 'Just') {
					var from = _v4.a.from;
					var to = _v4.a.to;
					if ($elm$core$String$isEmpty(
	...
	function (msg, model) {
		switch (msg.$) {
						...
						if (maybeTransition.$ === 'Just') {
							...
							return _Utils_update(
								model,
								{
									...
									consoleMessages: A2(
										$elm$core$List$cons,
										{msgType: $author$project$Components$Console$Error, text: 'Chyba: Neexistuje prechod pre symbol \'' + (symbol + '\'')},
										model.consoleMessages),
			...
			case 'ResetSimulation':
				return $author$project$Pages$Simulator$init(model.automaton);
			case 'AutoRun':
				return _Utils_update(
					model,
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'EditorMsg':
				var editorMsg = msg.a;
				if (editorMsg.$ === 'SwitchToSimulator') {
...
var $author$project$Pages$Editor$SetTransitionDisplayMode = function (a) {
	return {$: 'SetTransitionDisplayMode', a: a};
};
...
var $author$project$Components$AutomatonDisplay$viewFormalTransitionRow = F2(
...
var $author$project$Components$AutomatonDisplay$viewFormalTransitions = F2(
	function (states, transitions) {
		return A2(
			$elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
				[
					$elm$core$List$isEmpty(transitions) ? A2(
					$elm$html$Html$p,
					_List_fromArray(
						[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('Žiadne prechody')
						])) : A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
					...
					A2(
						$elm$core$List$map,
						$author$project$Components$AutomatonDisplay$viewFormalTransitionRow(states),
						transitions))
				]));
	});
var $author$project$Components$AutomatonDisplay$Formal = {$: 'Formal'};
...
var $author$project$Components$AutomatonDisplay$viewModeButton = F3(
	function (label, mode, config) {
		var isActive = _Utils_eq(config.displayMode, mode);
		var textColor = isActive ? 'white' : '#2c3e50';
		var bgColor = isActive ? '#3498db' : '#ecf0f1';
		return A2(
...
var $author$project$Components$AutomatonDisplay$viewTransitionControls = function (config) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				A3($author$project$Components$AutomatonDisplay$viewModeButton, 'Prechody', $author$project$Components$AutomatonDisplay$Table, config),
				A3($author$project$Components$AutomatonDisplay$viewModeButton, 'Formálny zápis', $author$project$Components$AutomatonDisplay$Formal, config)
			]));
};
...
var $author$project$Components$AutomatonDisplay$viewTransitionRow = F2(
	function (states, transition) {
		var toLabel = A2($author$project$Utils$AutomatonHelpers$getStateLabel, transition.to, states);
		var fromLabel = A2($author$project$Utils$AutomatonHelpers$getStateLabel, transition.from, states);
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(toLabel)
						]))
				]));
	});
var $author$project$Components$AutomatonDisplay$viewTransitions = F2(
	function (states, transitions) {
		return A2(
			$elm$html$Html$div,
			_List_Nil,
				...
				[
					...
					_List_fromArray(
						[
										...
										[
											...
											_List_fromArray(
												[
													A2($elm$html$Html$Attributes$style, 'border', '1px solid #ddd'),
													A2($elm$html$Html$Attributes$style, 'padding', '8px'),
													A2($elm$html$Html$Attributes$style, 'background-color', '#3498db'),
							...
							A2(
								$elm$core$List$map,
								$author$project$Components$AutomatonDisplay$viewTransitionRow(states),
								transitions))
						]))
				]));
	});
var $author$project$Components$AutomatonDisplay$view = function (config) {
	...
	var typeColor = isNFA ? '#e67e22' : '#3498db';
			...
			[
				...
				_List_fromArray(
					[
						...
						A2($elm$html$Html$Attributes$style, 'border-bottom', '2px solid #3498db'),
				...
				$author$project$Components$AutomatonDisplay$viewTransitionControls(config),
				function () {
				var _v0 = config.displayMode;
				if (_v0.$ === 'Table') {
					return A2($author$project$Components$AutomatonDisplay$viewTransitions, config.states, config.transitions);
				} else {
					return A2($author$project$Components$AutomatonDisplay$viewFormalTransitions, config.states, config.transitions);
				}
			}()
			]));
};
...
var $author$project$Components$Canvas$svgState = F2(
	function (config, state) {
		...
		var isActive = _Utils_eq(
			...
			$elm$core$Maybe$Just(state.id));
		var fillColor = isSelected ? '#6495ED' : (isTransitionStart ? '#FFD700' : (isActive ? '#90EE90' : '#E8E8E8'));
		var borderWidth = 3;
		var borderColor = isSelected ? '#00008B' : (isActive ? '#006400' : '#323232');
		return A2(
			$elm$svg$Svg$g,
			_List_fromArray(
				[
...
var $author$project$Components$Canvas$svgCurvedEdge = F5(
	function (config, a, b, symbols, isActive) {
		...
		var aw = 6;
		var al = 10;
		var baseX = tipX - (al * tUx);
		var leftX = baseX + ((aw / 2) * tPx);
		var rightX = baseX - ((aw / 2) * tPx);
		var baseY = tipY - (al * tUy);
		var leftY = baseY + ((aw / 2) * tPy);
		var rightY = baseY - ((aw / 2) * tPy);
		var arrowPts = A2(
			$elm$core$String$join,
			' ',
			_List_fromArray(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Attributes$style, 'border-top', '2px solid #34495e')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'background-color', '#2c3e50'),
...
var $author$project$Components$Toolbar$actionButton = F4(
	function (label, onClickMsg, isEnabled, bgColor) {
		return A2(
			$elm$html$Html$button,
			_List_fromArray(
				[
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$style,
					'background-color',
					isEnabled ? '#34495e' : '#7f8c8d'),
					A2(
					$elm$html$Html$Attributes$style,
					'color',
					isEnabled ? 'white' : '#bdc3c7'),
					A2($elm$html$Html$Attributes$style, 'border', 'none'),
					A2($elm$html$Html$Attributes$style, 'border-radius', '5px'),
					A2(
					$elm$html$Html$Attributes$style,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Attributes$style, 'background-color', '#2c3e50'),
				A2($elm$html$Html$Attributes$style, 'gap', '10px'),
				A2($elm$html$Html$Attributes$style, 'border-bottom', '2px solid #34495e'),
		...
		_List_fromArray(
			[
				...
				A4($author$project$Components$Toolbar$actionButton, 'Simulovať', config.onSwitchToSimulator, config.isSimulateEnabled, '#27ae60')
			]));
};
...
var $author$project$Pages$Editor$view = function (model) {
	...
	var isSimulateEnabled = A2($author$project$Utils$AutomatonHelpers$isDFA, states, transitions);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
...
var $author$project$Pages$Simulator$AutoRun = {$: 'AutoRun'};
...
var $author$project$Pages$Simulator$type_ = $elm$html$Html$Attributes$type_;
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$style,
					'background-color',
					isActive ? '#3498db' : (isEnabled ? '#34495e' : '#7f8c8d')),
					A2($elm$html$Html$Attributes$style, 'color', 'white'),
					A2($elm$html$Html$Attributes$style, 'border', 'none'),
					A2($elm$html$Html$Attributes$style, 'border-radius', '5px'),
					A2(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A4($author$project$Components$SimulateToolbar$toolButton, 'Krok vpred', config.onStepForward, config.canStepForward, false),
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Zostávajúci vstup: ')
							])),
						$elm$html$Html$text(
						$elm$core$String$isEmpty(config.remainingInput) ? '(prázdny)' : config.remainingInput)
					])),
				A2(
				$elm$html$Html$div,
				_List_Nil,
...
var $author$project$Pages$Simulator$view = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				{
					canStepBackward: !$elm$core$List$isEmpty(model.history),
					canStepForward: !$elm$core$String$isEmpty(model.remainingInput),
					isAutoRunning: model.isAutoRunning,
					onAutoRun: $author$project$Pages$Simulator$AutoRun,
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$author$project$Pages$Simulator$type_('text'),
...
var $author$project$Main$view = function (model) {
	...
	if (_v0.$ === 'EditorPage') {
		return A2(
			...
			$author$project$Pages$Editor$view(model.editorModel));
	} else {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
			...
			_List_fromArray(
				[
					...
					$author$project$Pages$Simulator$view(model.simulatorModel))
				]));
	}
};
